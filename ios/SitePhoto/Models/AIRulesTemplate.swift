...
enum AIRulesTemplate {
    ...
    Context names the overall purpose of the investigation — the job the engineer is doing on this project. Primary tag names the system, view, material, or feature category where we look for findings (e.g. a foundation grade beam, a masonry veneer wall, a site / yard view) — think of it as the *container* the specific finding lives in. Secondary tag names the specific thing observed inside that primary — either a distress (a crack, separation, displacement, spall, stain, distortion, poor connection, etc.) OR a documentation observation (an overall view of an elevation, a yard overview, an interior overview, a measurement readout). Many site photos show no distress at all; they're taken for overall-view documentation or orientation. The right answer for those is still a triple — a context-class primary plus an overall-view secondary — not an absent or low-confidence distress guess.
    ...
    - Expect a meaningful fraction of photos to be documentation / orientation only. The correct answer for those is a context-class primary (e.g. `General Exterior Orientation`, `General Interior Orientation`, `General Site Overview`) plus the most specific overall-view / context secondary listed under that primary (e.g. `Front elevation overall view`, `Interior overall view`, `Yard overview`). A documentation triple IS the right answer for a clean photo — it is not a fallback, and it is not a signal of low confidence. Do not invent or force a distress tag onto a clean overview photo.
