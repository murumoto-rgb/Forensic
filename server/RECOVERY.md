...
4. Pause mutating clients and let legacy upload URLs expire (up to 15 minutes). Apply 0016–0019, then deploy the matching server/web and provide the matching iOS build before resuming uploads. Old upload requests intentionally receive 426; an older client is not a safe substitute for this coordinated release. Keep local originals throughout the transition.
5. Verify non-admin privilege denial, frozen manifest/upload denial, config conflicts, save/restore conflicts, immutable upload/commit, required-file export failures, health checks and branding on the released versions. Keep paid AI calls and destructive restore/delete tests scoped to explicit test projects. See the implementation report for physical-device checks.

Rollback is not a schema downgrade. After v4 writes and immutable object registrations begin, reverting to an old server/client can lose new fields or break protected recovery. Prefer a forward fix; if a deployment must be reverted, first stop writes and use an intentionally compatible build. Retain the added database tables, upload receipts, version rows and all object bytes. Do not delete historical assets or run automatic garbage collection as rollback.
...
Concurrency qualification: the tests use single-connection PGlite. They inspect the installed commit function's lock ordering and execute both freeze-first and commit-first outcomes, including exact retries and access revocation. This workspace has libpq clients but no native PostgreSQL server binary, and Docker's daemon is unavailable. No true simultaneous-transaction stress result is claimed. `r2Read.test.ts` exercises stalled headers, a partially delivered/stalled body, cleanup, byte limits and truncation using fake transport and deterministic timers; no object-store request is made.
