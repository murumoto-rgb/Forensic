...
import { PendingSuggestionsReview } from "./PhotoSuggestionsReview";
import { AIAnalysisViewer } from "./PhotoAIAnalysisViewer";
import { ReTagWithAIControl } from "./PhotoRetagControl";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1 border-t border-neutral-800 px-4 py-3 text-sm">
        ...
        {/* Suggest / accept / reject pipeline (Build #5.41.1).
            Mirrors iOS's pending-suggestions chip row. Only renders
            when there ARE pending suggestions AND the parent wired
            up the update callback (the apply / reject buttons mutate
            the manifest via that callback). */}
        {onPhotoUpdated && photo.pendingSuggestions.length > 0 && (
        ...
        {/* Photo editor (Build #5.79.1 — Path P #4/8). Caption,
            observation, favorite, bucket, rotation, and direct tag
            add/remove — only renders when the parent wired an
            update callback (so view-only embeds stay view-only). */}
        {onPhotoUpdated && (
          <PhotoEditorSection
            project={project}
            photo={photo}
